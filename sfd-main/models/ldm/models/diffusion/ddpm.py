...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, step_condition=None):
        ...
        if hasattr(self, "split_input_params"):
            ...
            x_recon = self.model(x_noisy, t, **cond, step_condition=step_condition)

        # return x_recon  # for branch test
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, step_condition=None):
        if self.conditioning_key is None:
            out = self.diffusion_model(x, t, step_condition=step_condition)
        elif self.conditioning_key == 'concat':
            xc = torch.cat([x] + c_concat, dim=1)
            out = self.diffusion_model(xc, t)
        elif self.conditioning_key == 'crossattn':
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(x, t, context=cc, step_condition=step_condition)
